    ...
    def validate(self, world=None):
        ...
        for policy in self.tax_policies.values():
            ...
            if (policy.id not in world.society.polities or account is None
                    ...
                    or len(set(policy.embargoed_ids)) != len(policy.embargoed_ids)
                    or any(identity == policy.id or identity not in world.society.polities
                           for identity in policy.embargoed_ids)
                    or (policy.embargoed_ids and policy.embargo_policy_event_id is None)
                    or (policy.embargo_policy_event_id is not None and (
                        policy.embargo_policy_event_id not in events
                        or not any(delta.owner_kind == "tax_policy" and delta.owner_id == policy.id
                                   and delta.aspect == "embargoed_ids"
                                   and delta.after == ",".join(policy.embargoed_ids)
                                   for delta in events[policy.embargo_policy_event_id].deltas)))):
