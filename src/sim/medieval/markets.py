...
def update_markets(world):
    ...
    for market in sorted(economy.markets.values(), key=lambda m: m.id):
        ...
        prices = {}
        from .demand import construction_demand, research_demand
        for stock in economy.stocks.values():
            if stock.location_id == market.id:
                for rid in economy.resources:
                    demand[rid] += construction_demand(world, stock.id, rid) + research_demand(world, stock.id, rid)
...
def _purchase_terms(world, buy_id, sell_id):
    ...
    for field in ("quantity", "unit_price", "quote_day"):
        if type(terms[field]) is not int or terms[field] < (0 if field == "quote_day" else 1):
    ...
    if buyer.balance < terms["quantity"] * terms["unit_price"]:
...
def purchase(world, buy_decision_id, sell_decision_id):
    ...
    terms = _purchase_terms(world, buy_decision_id, sell_decision_id)
    ...
    event = record_event(candidate, "payment_completed", f"Compra à vista: {amount} unidades monetárias pagas.",
                         fact_kind=FactKind.STATE_TRANSITION,
                         deltas=(_delta("account", buyer.id, "balance", buyer.balance, buyer.balance - amount),
                                 _delta("account", seller.id, "balance", seller.balance, seller.balance + amount)),
                         cause_ids=_causes(buy_decision_id, sell_decision_id, order.last_event_id,
                                           buyer.last_event_id, seller.last_event_id))
    economy.accounts[buyer.id] = buyer.model_copy(update={"balance": buyer.balance - amount, "last_event_id": event.id})
    economy.accounts[seller.id] = seller.model_copy(update={"balance": seller.balance + amount, "last_event_id": event.id})
