...
| `.cursor/rules/map-system.mdc` | `src/run/map_*.py`, `src/run/load_map.py`, `src/classes/environment/**/*.py`, `src/server/services/game_queries.py`, `static/game_configs/maps/**/*.json`, `tools/map_presets/**/*.py`, `tools/map_creator/**/*.py`, `tests/test_map_*.py`, `tests/test_regional_routes.py`, `web/src/components/game/**/*.vue`, `web/src/types/**/*.ts` | O mapa oficial usa schema v6: `region_rows` é território semântico; `geography` é a verdade física; `Map.routes` é a rede explícita; `Map.infrastructure_sites` guarda sites espaciais sem copiar estado de outros donos; API e overlays apenas projetam esses registros; validar mapa, preview, save/load e renderer após mudanças. |
...
21. O mapa oficial vive em `static/game_configs/maps/<id>/map.json` schema v6: `region_rows` guarda território, `geography` guarda terreno/elevação/águas, `Map.routes` guarda conexões explícitas e `Map.infrastructure_sites` guarda sites espaciais sem duplicar estado de rotas, regiões ou organizações. Não restaurar `tile_map.csv/region_map.csv`, `wilderness_tile`, derivação de terreno por região ou rede espacial paralela; overrides locais devem sobreviver ao editor e save/load; após alterar presets rode ao menos `.venv/bin/python tools/map_presets/validate_presets.py`.
