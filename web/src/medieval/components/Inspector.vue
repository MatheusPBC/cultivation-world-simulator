...
const detachmentSieges=computed(()=>detachment.value ? data.value.campaigns.siege_campaigns.filter(item=>(item.phase==='sieging'||item.phase==='breached') && item.attacker_detachment_id===detachment.value!.id) : [])
  ...
  <aside class="inspector panel" data-testid="inspector">
    ...
    <div class="inspector-body">
      ...
      <template v-if="tab==='inspection'">
        ...
        <template v-else-if="detachment">
          <p class="eyebrow">{{t('detachments')}} · {{t('detachmentStages.' + detachment.stage)}}</p><h2>{{entityName(data,detachment.owner_ref)}}</h2>
          <p class="muted">{{detachment.id}}</p>
          <dl>
            <dt>{{t('owner')}}</dt><dd>{{entityName(data,detachment.owner_ref)}}</dd>
            <dt>{{t('detachmentCount')}}</dt><dd>{{n(detachment.count)}}</dd>
            <dt>{{t('location')}}</dt><dd>{{placeName(detachment.location_id)}}</dd>
            <dt>{{t('destination')}}</dt><dd>{{placeName(detachment.destination_id)}}</dd>
            <dt>{{t('provisions')}}</dt><dd>{{n(detachment.provisions)}}</dd>
            <dt>{{t('travelStage')}}</dt><dd>{{t('detachmentStages.' + detachment.stage)}}</dd>
            <dt>{{t('travelDue')}}</dt><dd>{{calendar(detachment.due_day)}}</dd>
          </dl>
          ...
          <template v-if="detachmentCommand">
            ...
            <article class="stock-card" :data-command="detachmentCommand.id">
              <p>{{data.society.characters.find(item=>item.id===detachmentCommand!.character_id)?.name ?? detachmentCommand.character_id}}</p>
              <p class="muted">{{t('office')}}: {{detachmentCommand.office_id}} · {{t('doctrine')}}: {{detachmentCommand.doctrine ? t('doctrines.' + detachmentCommand.doctrine) : t('none')}}</p>
              <button @click="source(detachmentCommand.last_event_id)">{{t('source')}}</button>
            </article>
          </template>
          <template v-if="detachmentPosition">
            <h3>{{t('preparedPosition')}}</h3>
            <article class="stock-card" :data-position="detachmentPosition.id">
              <p>{{placeName(detachmentPosition.settlement_id)}} · {{detachmentPosition.anchor_site_id ?? t('none')}}</p>
              <p class="muted">{{t('readyOn')}}: {{calendar(detachmentPosition.ready_day)}}</p>
              <button @click="source(detachmentPosition.last_event_id)">{{t('source')}}</button>
            </article>
          </template>
          <template v-if="detachmentStandoffs.length || detachmentGarrisons.length || detachmentSieges.length || detachmentInterdictions.length || detachmentInvestments.length || detachmentDenials.length">
            <h3>{{t('activeForceRelations')}}</h3>
            <article v-for="item in detachmentStandoffs" :key="item.id" class="stock-card" :data-standoff="item.id"><p>{{t('standoff')}} · {{placeName(item.settlement_id)}}</p><button @click="source(item.last_event_id)">{{t('source')}}</button></article>
            <article v-for="item in detachmentGarrisons" :key="item.id" class="stock-card" :data-garrison="item.id"><p>{{t('garrison')}} · {{placeName(item.settlement_id)}}</p><button @click="source(item.last_event_id)">{{t('source')}}</button></article>
            <article v-for="item in detachmentSieges" :key="item.id" class="stock-card" :data-siege="item.id"><p>{{t('siege')}} · {{placeName(item.settlement_id)}} · {{t('siegePhases.' + item.phase)}}</p><button @click="source(item.last_event_id)">{{t('source')}}</button></article>
            <article v-for="item in detachmentInterdictions" :key="item.id" class="stock-card" :data-interdiction="item.id"><p>{{t('routeInterdiction')}} · {{item.route_id}}</p><button @click="source(item.last_event_id)">{{t('source')}}</button></article>
            <article v-for="item in detachmentInvestments" :key="item.id" class="stock-card" :data-investment="item.id"><p>{{t('settlementInvestment')}} · {{placeName(item.settlement_id)}}</p><button @click="source(item.last_event_id)">{{t('source')}}</button></article>
            <article v-for="item in detachmentDenials" :key="item.id" class="stock-card" :data-assembly-denial="item.id"><p>{{t('assemblyDenial')}} · {{placeName(item.settlement_id)}}</p><button @click="source(item.last_event_id)">{{t('source')}}</button></article>
          </template>
        </template>
      ...
      <template v-if="tab==='governments'"><h2>{{t('governments')}}</h2><article v-for="p in data.society.polities" :key="p.id" class="stock-card" :data-polity="p.id"><h3>{{p.name}}</h3><p>{{label(p.government)}}</p><button class="link-row" @click="select('polity',p.id)">{{t('inspect')}} →</button><button class="link-row" @click="select('settlement',p.capital_id)">{{t('capital')}}: {{placeName(p.capital_id)}} →</button><p v-for="i in p.interests" :key="i" class="muted">{{i}}</p></article><h2>{{t('organizations')}}</h2><article v-for="o in data.society.organizations" :key="o.id" class="stock-card" :data-organization="o.id"><h3>{{o.name}}</h3><p>{{label(o.kind)}}</p><p v-for="i in o.interests" :key="i" class="muted">{{i}}</p><button class="link-row" @click="select('organization',o.id)">{{t('inspect')}} →</button><button @click="select('settlement',o.seat_id)">{{t('seat')}}: {{placeName(o.seat_id)}} →</button></article></template>
